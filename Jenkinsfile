pipeline {
    ...
    environment {
        NEXUS_REPOSITORY = "cma-application"
        NEXUS_VERSION = "nexus3"
        NEXUS_PROTOCOL = "https"
        NEXUS_URL = "nexus.globalfrontierlink.co.uk"
        NEXUS_CREDENTIAL_ID = "nexus-administrator"
        SONARQUBE_SERVER = 'sonarqube.globalfrontierlink.co.uk'
    }
    ...
    stages {
        stage('Clone CMA Application') {
            steps {
                sh """
                git clone git@github.com:emmanuelcodes2020/cma-app-customer-management.git
                pwd
                """
            }
        }
